...
pub fn toggle_settings_ui(keyboard: Res<ButtonInput<KeyCode>>, mut ui_state: ResMut<SettingsUiState>, mut editing_state: ResMut<KeybindEditingState>) {
	if editing_state.editing_field.is_some() {
		if keyboard.just_pressed(KeyCode::Escape) { editing_state.editing_field = None; }
		return;
	}
	...
	if ui_state.page == SettingsUiPage::MainMenu {
		egui::Window::new(t("settings.title")).default_size([260.0, 180.0]).collapsible(false).resizable(false).show(ctx, |ui| {
			ui.vertical_centered(|ui| {
				...
				ui.add_space(10.0);
				if ui.add_sized([180.0, 44.0], egui::Button::new(t("settings.quit_game"))).clicked() {
...
fn draw_keybind_row(ui: &mut egui::Ui, label: &str, binding: &mut InputBinding, field: KeybindField, editing_state: &mut ResMut<KeybindEditingState>, keyboard: &Res<ButtonInput<KeyCode>>, mouse: &Res<ButtonInput<MouseButton>>, ) {
	...
	ui.horizontal(|ui| {
		...
		let button_text = if is_editing { "Press key or mouse...".to_string() } else { input_binding_to_string(*binding) };
		let response = ui.button(&button_text);
		if response.clicked() && !is_editing { editing_state.editing_field = Some(field); }
	});
...
fn collect_keybind_conflicts(keybinds: &PlayerKeybinds, i18n: &I18nResource) -> Vec<String> {
	let entries = [
		(i18n.t("settings.move_up"), keybinds.move_up), (i18n.t("settings.move_down"), keybinds.move_down),
		(i18n.t("settings.move_left"), keybinds.move_left), (i18n.t("settings.move_right"), keybinds.move_right),
		(i18n.t("settings.jump"), keybinds.jump), (i18n.t("settings.interact"), keybinds.interact),
		(i18n.t("settings.attack"), keybinds.attack), (i18n.t("settings.shoot"), keybinds.shoot),
...
fn detect_any_input_press(keyboard: &Res<ButtonInput<KeyCode>>, mouse: &Res<ButtonInput<MouseButton>>) -> Option<InputBinding> {
	let candidates = [
		...
		KeyCode::Space, KeyCode::Tab, KeyCode::ArrowUp, KeyCode::ArrowDown, KeyCode::ArrowLeft, KeyCode::ArrowRight,
	];
	for key in candidates { if keyboard.just_pressed(key) { return Some(InputBinding::Keyboard(key)); } }
	let mouse_candidates = [MouseButton::Left, MouseButton::Right, MouseButton::Middle, MouseButton::Back, MouseButton::Forward];
	for button in mouse_candidates { if mouse.just_pressed(button) { return Some(InputBinding::Mouse(button)); } }
	None
}
