...
use super::types::*;
use super::ui::{draw_settings_ui, toggle_settings_ui};
            ...
            VleueSide::Client => {
                ...
                app.add_plugins(FpsOverlayPlugin {
                    config: FpsOverlayConfig {
                        enabled: settings.graphics.show_fps,
                        text_config: TextFont {
                            font_size: 16.0,
                            ..default()
                        },
                        text_color: Color::srgb(0.8, 0.92, 0.78),
                        ..default()
                    },
                });
                if settings.debug.show_inspector {
...
fn log_render_adapter_info(adapter_info: Res<RenderAdapterInfo>) {
    info!(
        "Render adapter: backend={:?}, name={}, device_type={:?}, driver={}, driver_info={}",
...
fn sync_fps_overlay_config(settings: Res<GameSettings>, mut overlay_config: ResMut<FpsOverlayConfig>, ) {
    if !settings.is_changed() || overlay_config.enabled == settings.graphics.show_fps {
        return;
    }
...
pub fn apply_window_settings(settings: Res<GameSettings>, 	mut windows: Query<&mut Window, With<PrimaryWindow>>, ) {
    ...
    window.mode = match settings.window.window_mode {
